...
import { Search, Edit2, Menu, Bell, ChevronDown, Users, Settings, User } from 'lucide-react';
...
const AdminDashboard = () => {
  ...
  return (
    <div className="flex h-screen bg-gray-100">
      ...
      <aside className={`bg-indigo-700 text-white w-64 min-h-screen p-4 ${sidebarOpen ? 'block' : 'hidden'} md:block`}>
        ...
        <nav>
          <a href="#" className="flex items-center py-2 px-4 text-gray-100 hover:bg-indigo-600 rounded">
            <Users className="mr-3" size={20} />
            <span>Users</span>
          </a>
          <a href="#" className="flex items-center mt-2 py-2 px-4 text-gray-100 hover:bg-indigo-600 rounded">
      ...
      <div className="flex-1 flex flex-col overflow-hidden">
        ...
        <main className="flex-1 overflow-x-hidden overflow-y-auto bg-gray-100">
          <div className="container mx-auto px-6 py-8">
            <h3 className="text-gray-700 text-3xl font-medium mb-6">User Management</h3>
                    ...
                    <tr key={user._id} className={index % 2 === 0 ? 'bg-gray-50' : 'bg-white'}>
                      ...
                      <td className="px-6 py-4 whitespace-nowrap text-sm font-medium">
                        <button
                          ...
                          className={`text-${user.status === "Blocked" ? "green" : "red"}-600 hover:text-${user.status === "Blocked" ? "green" : "red"}-900 mr-3`}
                        >
                          {user.status === "Blocked" ? "Unblock" : "Block"}
                        </button>
                        <button className="text-indigo-600 hover:text-indigo-900" onClick={()=>navigate((`/admin/useredit/${user._id}`))} >
                          <Edit2 size={16} />
                        </button>
                      </td>
                    </tr>
